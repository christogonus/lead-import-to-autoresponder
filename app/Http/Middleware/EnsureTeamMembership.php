...
class EnsureTeamMembership
{
    ...
    public function handle(Request $request, Closure $next, ?string $minimumRole = null): Response
    {
        ...
        // The requested team doesn't exist or the user isn't a member: send them
        // to a picker of their own teams instead of a hard 403.
        if (! $team || ! $user->belongsToTeam($team)) {
            return redirect()->route('teams.select')->with(
                'status',
                __("You don't have access to that team. Choose one of your teams below."),
            );
        }
